...
const API_URL = API?.CARROS_WRITE_URL || "https://webmotors-clone-back.onrender.com/carros";
...
async function carregarCarros() {
  try {
    ...
    carros.forEach((carro) => {
      ...
      let imagemUrl = "/assets/sem-imagem.webp"; // fallback absoluto
      if (imagens && imagens.length > 0) {
        const img = imagens[0];
        if (!img) {
          imagemUrl = "/assets/sem-imagem.webp";
...
document.getElementById("emPromocao")?.addEventListener("change", togglePrecoAntigo);
...
function iniciarEdicao(id) {
  ...
  document.getElementById("aceitaTroca").value = carro.aceitaTroca ? "true" : "false";
...
formData.append("badge", document.getElementById("badge").value);
    ...
    formData.append("descricaocurta", document.getElementById("descricaoCurta").value);
    formData.append("aceitatroca", document.getElementById("aceitaTroca").value);
...
async function renderPreview() {
  ...
  const maxVisible = 4;
  const filesToShow = showingAll ? currentFiles : currentFiles.slice(0, maxVisible);
  ...
  filesToShow.forEach((item, index) => {
    ...
    if(!showingAll && index === maxVisible -1 && currentFiles.length > maxVisible){
